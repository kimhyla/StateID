...
router.post('/record', (req, res) => {
  ...
  if (!sessionId || !status) return res.status(400).json({ error: 'sessionId and status required' });
  ...
  db.prepare(`INSERT INTO attendee_events (
    ...
    rowId,
    sessionId,
    attendeeSlotHash || null,
    status,
    state || null,
    selfDeclaredState || null,
    method || null,
    withinScope || 'unknown',
    overrideReason || null,
    overrideTimestampUtc || null,
    noteText || null,
  );
