...
• **Zoom** (when connected):
  - Prepend one ultra-short verify line to the meeting agenda/description via API (flows into Zoom emails/ICS).
  - If verification is still missing at host start, quietly add the short verify link to the Waiting Room message.
...
• Portal-only flows (no visible URL): handled via Your Join Link (if using a fixed room) or the therapist banner (Ask / Assign a State / Acknowledge).
• Static room per therapist: Your Join Link routes all clicks through StateID.

**Performance budget**
• Client click budget ~100–200 ms. If exceeded or any error: fail open (immediate redirect to original); server performs 3 background retries (250ms → 750ms → 2s; stop <3 minutes total).
...
→ Ledger when Approved: status=assumed_from_prior, method=assumed (updates the existing row).
...
• Every resolved status is checked against Allowed States (+ PSYPACT).
  - In-scope: silent success.
  - Out-of-scope: show small warning banner (see §7).
...
• Enter client's current State: dropdown → ledger = Provider Assigned.
...
• If no attendee emails (portal-only), session-level status with optional manual per-attendee chips.
...
• **Named Audit Pack** (local, on your computer) — the export.
  - Merges calendar names locally; outputs named PDF/CSV; no PHI leaves device.
  - Short explainer: "Names are added on your device using your calendar; nothing leaves your computer."
  - If a calendar event lacks names (portal-only), the line remains anonymized.
...
• Server background 3 retries with short backoff (250ms → 750ms → 2s; stop <3 minutes total).
...
• **AttendeeEvent** {session_id, attendee_slot_hash, status, state, self_declared_state, method, within_scope, override_reason, override_timestamp_utc, timestamp_utc, note_text}
...
• Latency SLOs: Wrapper p95 ≤ 200 ms across key regions; fail-open rate < 3% (rolling 7 days).
...
• **V1 options** (optional restores):
  - Outlook OnAppointmentSend add-in (org deploy).
  - iOS/Android Share extensions ("Share → StateID Wrap").
  - Vanity domains/CNAME for Join Links.
...
• **Group sessions**: per-attendee chips when emails present; one Ask covers all; if no emails, session-level status with optional manual per-attendee chips.
• **Yahoo-only manual events**: use Your Join Link or banners.
• **Non-client sessions**: "Not a client session — do not log" → soft-delete with minimal deletion marker.
...
• Parallel geo lookups; evaluate against the two-tier confidence rule (do not stop early in the 0.60–0.79 band).
...
1. Clinician creates an appointment as usual (Google/Outlook; or Zoom creates it).
2. StateID quietly wraps the join link. If Zoom is connected, we also set a tiny agenda line (and, only if needed, a Waiting Room link); Teams shows a tiny line in Details.
3. Client taps the invite at session time.
4. Our edge runs the near-instant check (~0.1–0.2s) and deep-links/302s to Zoom/Meet. If it would take longer, we fail open and finish in the background.
5. In the desktop app, the clinician sees: Verified, or Out-of-scope (Acknowledge/Correct the State), or—if detection failed—either the Assume toast or the Ask banner.
6. Later (or monthly), they open Exports and generate the Named Audit Pack (local) for an insurer/board.